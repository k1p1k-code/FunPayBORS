...
use crate::{models_reqwest, models_response};
use async_zip::tokio::read::seek::ZipFileReader;
use axum::extract::Multipart;
use axum::{Json, extract::State as StateAxum};
use models::{AppState, EventServer};
use python_plugins::utils as python;
use python_plugins::{delete_plugin, install_plugin};
use std::io::Cursor;
use std::sync::Arc;
...
pub async fn install_plugin_web(
    ...
    match install_plugin(file_byte, plugin_name) {
        Ok(_) => reload_plugins(StateAxum(tx.into())).await,
        Err(e) => Json(Response {
...
pub async fn callback_plugin(
    ...
    if let Some(plugin) = plugin_check {
        if let Some(build_menu) = &plugin.build_menu {
            let menu = python::run_menu_build(build_menu, &plugin.storage)
                .await
                .expect(format!("cannot build menu in {}", plugin.name.to_string()).as_str());
            if let Some(buttons) = menu.button
                && plugin_callback.callback_type == "button".to_string()
            {
                        ...
                        .await
                        {
                            Ok(s) => {
                                if s.is_empty() {
                                    Json(Response {
                                        message: Some(
                                            "Successful callback(no message)".to_string(),
                                        ),
...
pub async fn list_plugins(
    StateAxum(app_state): StateAxum<Arc<Mutex<AppState>>>,
) -> Json<Vec<models_response::ResponseListPlugins>> {
    ...
    for i in plugins.iter() {
        ...
        if let Some(menu) = &i.build_menu {
            let menu = match python::run_menu_build(menu, &i.storage).await {
                Ok(s) => s,
                Err(_) => {
                    texts.push("Bad menu".to_string());
                    result.push(models_response::ResponseListPlugins {
                        name: i.name.clone(),
                        error: None,
                        texts,
            ...
            // let menu=python::run_menu_build(menu, &i.storage).await.expect(format!("cannot build menu in {}", i.name.to_string()).as_str()); fuck my code
